// visibility
#define PLAYER_DRAW_SIZE 0.5
#define PLAYER_DELTA_DRAW_POS dot(-30.5, 41) * PLAYER_DRAW_SIZE
...
struct Player {
	...
	void draw() const {
		...
		// draw hp
		for (int i = 0; i < hp; i++) {
			draw_sprite(dot(-arena_half_size.x, arena_half_size.y) * 0.95 + camera.pos + dot(i, 0) * 7, 0.5, SP_HEART);
		}
